...
import { useSentRequests } from "../../store/actions/contacts"
import { useCallback, useEffect, useRef, useState } from "react"
import { countFollowingRequestsPaginationPage } from "../../store/reducers/contacts"

export default ({}) => {
    const [loadingContacts, setLoadingContacts] = useState(false)
    const contacts = useSelector((state) => state.contacts)
    const dispatch = useDispatch()
    const {handleSentRequests, handlePaginatedSentRequests} = useSentRequests()
    ...
    const lastContactRef = useCallback(node => {
        ...
        observer.current = new IntersectionObserver(entries => {
            if(entries[0].isIntersecting) {
                dispatch(countFollowingRequestsPaginationPage(contacts.getFollowingRequestsPagination.page + 1))

                if(contacts.getFollowingRequestsPagination.page < contacts.getFollowingRequestsPagination.last_page) {
                    setLoadingContacts(prevLoadingContacts => prevLoadingContacts = true)
                    handlePaginatedSentRequests(contacts.getFollowingRequestsPagination.page + 1).then(() => {
    ...
    }, [contacts.getFollowingRequestsPagination.page, contacts.getFollowingRequestsPagination.last_page, loadingContacts])

    useEffect(() => {
        setLoadingContacts(prevLoadingContacts => prevLoadingContacts = true)
        handleSentRequests(contacts.getFollowingRequestsPagination.page).then(() => {
            setLoadingContacts(prevLoadingContacts => prevLoadingContacts = false)
        })
    }, [contacts.getFollowingRequestsPagination.page, contacts.getFollowingRequestsPagination.last_page])
    ...
    return(
        <>
            <div className="block border-b border-black border-opacity-5 last:border-none dark:border-milky-white dark:border-opacity-10">
                {
                    contacts.sentRequests && contacts.sentRequests.length > 0
                    ?
                    contacts.sentRequests.map((request, index) => {
                        if(contacts.sentRequests.length === index + 1) {
                    ...
                    : <strong className="w-full max-w-[250px] relative m-auto p-4 text-center flex justify-center items-center text-black text-opacity-50 dark:text-milky-white dark:text-opacity-50">
                        You haven't sent any following requests yet.
                    </strong>
                }
            </div>
            {
